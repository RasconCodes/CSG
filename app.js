...
function createStamp() {
  ...
  const size =
    Number(stampSizeInput.value);

  const depth =
    Number(letterDepthInput.value);

  const handleHeight =
    Number(handleHeightInput.value);

  const engravingDepth =
    Number(engravingDepthInput.value);
